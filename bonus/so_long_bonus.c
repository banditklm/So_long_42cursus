...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/02/16 23:12:21 by kelmounj          #+#    #+#             */
/*   Updated: 2024/02/26 21:57:46 by kelmounj         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	main(int ac, char **av)
{
	...
	data.mlx_win = mlx_new_window(data.mlx_ptr,
			map.width * 60, map.height * 60, "so_long_bonus");
	if (!(data.mlx_win))
		ft_error("mlx fails!", &map, NULL, (&map)->map);
	...
	image_init(&data);
	fill_map(data);
	mlx_loop_hook(data.mlx_ptr, put_animation, &data);
